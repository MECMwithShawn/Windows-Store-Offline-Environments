...
4. `Remove-MECMStoreApp.ps1` can remove deployments, Application Groups,
...
| Privilege | MODERATE | Download can run unprivileged; publish, distribution, and cleanup operations require MECM and share permissions. |
...
Selecting an application or Application Group derives its associated frameworks
from MECM `SDMPackageXML`, labels them exclusive or shared, lists non-selected
consumers, and asks whether eligible frameworks should join the removal plan.
The resolved plan then displays groups, applications, and frameworks separately
and requires the operator to type `REMOVE` exactly unless `-WhatIf` or `-Force`
is used. `-FrameworkName` provides explicit framework targeting. Frameworks used
by non-selected objects are warned and excluded unless the high-risk
`-RemoveSharedFrameworks` override is present. Redirected input cannot bypass
...
The publish and cleanup stages have no deliberate Internet dependency.
...
2. Record SHA-256 hashes for the three scripts and the complete transfer set.
...
| Change preview | Publisher and cleanup tool implement `ShouldProcess`; `-WhatIf` is available. |
...
| Removal plan confirmation | Exact resolved objects are grouped by type; interactive deletion requires the literal `REMOVE` token and noninteractive deletion requires explicit `-Force`. |
| Shared-framework protection | Framework references in non-selected applications and groups are reported and blocked unless `-RemoveSharedFrameworks` is explicitly approved. |
...
The publisher's icon workflow extracts data beneath `%TEMP%\MECMIcons` and may temporarily
...
all bounded to the owned inventory. The final plan requires exact confirmation,
and shared frameworks are blocked by default when non-selected consumers are
detected. Regression tests confirm that Notepad++, unrelated groups, and shared
...
- [ ] Review all three PowerShell files line-by-line.
