...
Selecting an application or Application Group automatically resolves associated
frameworks from MECM application/group XML. Each framework is labeled
`EXCLUSIVE` or `SHARED`; non-selected consumers are listed, and the operator is
asked whether eligible associated frameworks should join the deletion plan.
The final plan separates Application Groups, applications, and frameworks and
requires the operator to type `REMOVE` exactly. For targeted cleanup, use
`-AppName` for applications and `-FrameworkName` for dependency frameworks; the
interactive menu exposes frameworks as `FW1`, `FW2`, and so on. If a selected
framework is referenced in the `SDMPackageXML` of any non-selected application
or Application Group, the script lists those consumers and removes the framework
from the deletion plan. `-RemoveSharedFrameworks` overrides this protection and
should require a separately approved change. Unattended execution without
`-WhatIf` must use `-Force`; redirected input cannot silently bypass confirmation.
